...
export default function SocialMediaCropsPage() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/specialty/social-crops`, { method: 'POST', body: formData })
      if (!res.ok) {
      ...
      a.href = url; a.download = 'social-media-crops.zip'
      document.body.appendChild(a); a.click()
      URL.revokeObjectURL(url); document.body.removeChild(a)
      setSuccess(true); setFile(null)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to create social media crops. Please try again.')
    } finally {
      setProcessing(false)
    }
  }
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Social Media Crops</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Auto-crop one image to every major social media format at once</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'20px auto 0', padding:'0 24px' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))', gap:'10px' }}>
          {platforms.map(p => (
            <div key={p.name} style={{ background:'#f8fafc', border:'1.5px solid #e2e8f0', borderRadius:'8px', padding:'10px 12px' }}>
              <div style={{ fontSize:'13px', fontWeight:600, color:'#0F2A4A' }}>{p.name}</div>
              <div style={{ fontSize:'11px', color:'#94a3b8', fontFamily:'monospace', marginTop:'2px' }}>{p.size}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'16px auto 0', padding:'0 24px' }}>
        <div
          ...
          <div style={{ fontSize:'12px', color:'#94a3b8', marginTop:'12px' }}>JPG, PNG, WebP · Max 20 MB · High-res recommended</div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Done! Your social-media-crops.zip has downloaded — 8 platform-ready images inside.</div>}
        ...
        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={convert} disabled={!file || processing}
            ...
            {processing ? '⏳ Creating crops…' : '📱 Create Social Media Crops'}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How It Works</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your image (high resolution is best — at least 1280px wide).','Click "Create Social Media Crops" — 8 versions are generated at once.','Download the ZIP file containing all platform-specific sizes.','Upload each file to the appropriate social platform.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'Does it crop or just resize?', a:'The tool crops and resizes to fit each exact platform dimension. Center-based cropping is used to preserve the most important part of your image.' },
            { q:'What is the minimum input image size?', a:'For best results, upload an image that is at least 1280×1280px. Smaller images will still work but may appear pixelated on larger formats like YouTube thumbnails.' },
            { q:'Are social media sizes up to date?', a:'Sizes are based on current recommended dimensions as of 2026. Platforms sometimes change their recommendations — always double-check for critical posts.' },
