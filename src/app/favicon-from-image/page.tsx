...
export default function FaviconFromImagePage() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/specialty/favicon`, { method: 'POST', body: formData })
      if (!res.ok) {
      ...
      setError(err instanceof Error ? err.message : 'Failed to generate favicons. Please try again.')
    } finally {
      setProcessing(false)
    }
  }
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            ...
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Generate all favicon sizes from any image — includes HTML snippet</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'20px auto 0', padding:'0 24px' }}>
        <div style={{ background:'#EFF6FF', border:'1.5px solid #BFDBFE', borderRadius:'12px', padding:'14px 18px' }}>
          ...
          <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' as const }}>
            {sizes.map(s => (
              <span key={s} style={{ background:'#DBEAFE', color:'#1E40AF', fontSize:'12px', fontWeight:600, padding:'3px 8px', borderRadius:'4px' }}>{s}×{s}px</span>
            ))}
          </div>
          <div style={{ fontSize:'13px', color:'#1E3A8A', marginTop:'8px', lineHeight:'1.5' }}>
            Plus a <code style={{ background:'#DBEAFE', padding:'1px 4px', borderRadius:'3px' }}>html-snippet.txt</code> with ready-to-paste <code style={{ background:'#DBEAFE', padding:'1px 4px', borderRadius:'3px' }}>&lt;link&gt;</code> tags. A square image (1:1) gives the best results.
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'16px auto 0', padding:'0 24px' }}>
        <div
          ...
          <div style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'20px', fontWeight:700, color:'#0F2A4A', marginBottom:'6px' }}>Drop your logo or image here</div>
          ...
          <div style={{ fontSize:'12px', color:'#94a3b8', marginTop:'12px' }}>JPG, PNG, WebP · Square image recommended · Max 10 MB</div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Done! Your favicons.zip has downloaded — unzip and add to your website.</div>}
        ...
        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={convert} disabled={!file || processing}
            ...
            {processing ? '⏳ Generating favicons…' : '⭐ Generate Favicons'}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Generate Favicons</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your logo or icon image (square works best).','Click "Generate Favicons" — all sizes are created instantly.','Download favicons.zip and extract the files.','Copy the HTML snippet into your <head> tag and upload icons to your site root.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'Why do I need so many sizes?', a:'Different platforms use different sizes: browsers use 16×16 and 32×32, iOS uses 180×180, Android uses 192×192, and app stores use 512×512.' },
            { q:'Does my image need to be square?', a:'It does not have to be, but square images give the best results since favicons are displayed in square/circular frames.' },
            { q:'Where do I put the favicon files?', a:'Upload the favicon files to your website\'s root directory and paste the HTML snippet into your <head> tag.' },
