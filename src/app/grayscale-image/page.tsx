...
export default function GrayscaleImagePage() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/image/grayscale`, { method: 'POST', body: formData })
      if (!res.ok) {
      ...
      a.href = url; a.download = 'grayscale.jpg'
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Grayscale Image</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Convert a colour image to black and white with one click</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        <div
          ...
          <div style={{ fontSize:'12px', color:'#94a3b8', marginTop:'12px' }}>JPG, PNG, WebP · Max 20 MB · Output is grayscale.jpg</div>
        ...
        {file && (
          <div style={{ marginTop:'20px', background:'white', border:'1.5px solid #e2e8f0', borderRadius:'10px', padding:'16px', display:'flex', alignItems:'center', gap:'12px' }}>
            <div style={{ width:'40px', height:'40px', background:'#f1f5f9', borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center', color:'#475569', fontWeight:700, fontSize:'10px', flexShrink:0 }}>IMG</div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Converted to grayscale! Your grayscale.jpg has downloaded.</div>}
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section>
          ...
          {[
            { q:'What colour algorithm is used?', a:'We use the luminosity method — a weighted average of RGB channels (0.21R + 0.72G + 0.07B) that matches how the human eye perceives brightness. This gives more natural-looking results than simple averaging.' },
            { q:'Can I convert back to colour?', a:'No — colour information is discarded when converting to grayscale. The process is irreversible.' },
            { q:'What is the output format?', a:'The output is a JPG file. PNG output is planned.' },
