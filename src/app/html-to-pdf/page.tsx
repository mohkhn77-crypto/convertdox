...
export default function HtmlToPdfPage() {
  ...
  const handleFile = (f: File | null) => {
    if (!f) return
    if (!f.name.match(/\.html?$/i) && f.type !== 'text/html') {
      setError('Only HTML files (.html, .htm) are accepted'); return
    }
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/pdf/html-to-pdf`, { method: 'POST', body: formData })
      if (!res.ok) {
      ...
      a.href = url; a.download = 'from-html.pdf'
      document.body.appendChild(a); a.click()
      URL.revokeObjectURL(url); document.body.removeChild(a)
      setSuccess(true); setFile(null)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to convert HTML. Please try again.')
    } finally {
      setProcessing(false)
    }
  }
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>HTML to PDF</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Convert HTML files to PDF — preserves CSS styling and layout</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        <div
          ...
          <div style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'20px', fontWeight:700, color:'#0F2A4A', marginBottom:'6px' }}>Drop your HTML file here</div>
          <div style={{ fontSize:'14px', color:'#64748b', marginBottom:'18px' }}>or click to browse from your computer</div>
          <button style={{ background:'#E85D04', color:'white', padding:'12px 32px', borderRadius:'10px', border:'none', fontSize:'14px', fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>Select HTML File</button>
          <div style={{ fontSize:'12px', color:'#94a3b8', marginTop:'12px' }}>.html and .htm files · Max 10 MB</div>
          <input ref={fileInputRef} type="file" accept=".html,.htm,text/html" style={{ display:'none' }} onChange={e => handleFile(e.target.files?.[0] ?? null)} />
        </div>
        ...
        {file && (
          <div style={{ marginTop:'20px', background:'white', border:'1.5px solid #e2e8f0', borderRadius:'10px', padding:'16px', display:'flex', alignItems:'center', gap:'12px' }}>
            <div style={{ width:'40px', height:'40px', background:'#FEF3C7', borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center', color:'#92400E', fontWeight:800, fontSize:'11px', flexShrink:0 }}>HTML</div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Converted! Your PDF has downloaded.</div>}
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Convert HTML to PDF</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your .html or .htm file.','Click "Convert to PDF".','Our server renders the HTML using a headless browser — CSS and images in the same directory are applied.','Your PDF downloads automatically.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'Will external CSS and images load?', a:'External URLs referenced in the HTML will be fetched if they are publicly accessible. Local file:// references will not load.' },
            { q:'Is JavaScript executed?', a:'Yes — the headless browser runs JavaScript before capturing the page, so dynamic content is included.' },
            { q:'What page size is used?', a:'A4 portrait by default. Custom page sizes are planned.' },
            { q:'Is there a file size limit?', a:'HTML files up to 10 MB are supported.' },
          ].map(faq => (
