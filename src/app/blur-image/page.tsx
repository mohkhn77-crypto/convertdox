...
export default function BlurImagePage() {
  ...
  const blur = async () => {
    ...
    try {
      ...
      formData.append('blur', String(blurAmount))
      const res = await fetch(`${BACKEND_URL}/api/image/blur`, { method: 'POST', body: formData })
      if (!res.ok) {
      ...
      a.href = url; a.download = 'blurred.jpg'
      document.body.appendChild(a); a.click()
      URL.revokeObjectURL(url); document.body.removeChild(a)
      setSuccess(true); setFile(null)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to blur image. Please try again.')
    } finally {
      setProcessing(false)
    }
  }
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          <div style={{ width:'56px', height:'56px', background:'rgba(100,116,139,0.2)', borderRadius:'14px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'28px' }}>🌫</div>
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Blur Image</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Apply Gaussian blur to an image — great for hiding sensitive information</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        ...
        <div style={{ marginTop:'24px', background:'#f8fafc', border:'1.5px solid #e2e8f0', borderRadius:'12px', padding:'20px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
            <label style={{ fontSize:'13px', fontWeight:700, color:'#0F2A4A' }}>Blur Amount</label>
            <span style={{ fontSize:'14px', fontWeight:700, color:'#E85D04' }}>{blurAmount}</span>
          </div>
          <input type="range" min={1} max={20} value={blurAmount} onChange={e => setBlurAmount(Number(e.target.value))}
            style={{ width:'100%', accentColor:'#E85D04' }} />
          <div style={{ display:'flex', justifyContent:'space-between', fontSize:'11px', color:'#94a3b8', marginTop:'4px' }}>
            <span>Subtle</span><span>Heavy blur</span>
          </div>
        </div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Done! Your blurred image has downloaded.</div>}

        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={blur} disabled={!file || processing}
            ...
            {processing ? '⏳ Blurring image…' : '🌫 Blur Image'}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Blur an Image</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your image.','Adjust the blur amount slider (1 = subtle, 20 = heavy blur).','Click "Blur Image" to apply.','Download your blurred image.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'What algorithm is used for blurring?', a:'Gaussian blur — a widely-used algorithm that produces smooth, natural-looking blur across the entire image.' },
            { q:'Can I blur only part of an image?', a:'Currently the blur is applied to the entire image. Selective region blurring is planned for a future update.' },
            { q:'What blur level should I choose?', a:'Level 3–5 gives a soft blur. Level 10–15 significantly obscures content. Level 20 makes most details unrecognizable.' },
