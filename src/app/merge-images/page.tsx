...
export default function MergeImagesPage() {
  ...
  const handleFiles = (newFiles: FileList | null) => {
    if (!newFiles) return
    const imgs = Array.from(newFiles).filter(f => ACCEPTED_TYPES.includes(f.type))
    if (imgs.length !== newFiles.length) setError('Only image files (JPG, PNG, WebP, GIF, BMP) are accepted')
    else setError('')
  ...
  const mergeImages = async () => {
    if (files.length < 2) { setError('Please upload at least 2 images'); return }
    setMerging(true); setError(''); setSuccess(false)
    try {
      const formData = new FormData()
      files.forEach(f => formData.append('files', f))
      formData.append('direction', direction)
      const res = await fetch(`${BACKEND_URL}/api/images/merge`, { method: 'POST', body: formData })
      if (!res.ok) {
      ...
      a.href = url; a.download = `merged.${ext}`
      document.body.appendChild(a); a.click()
      URL.revokeObjectURL(url); document.body.removeChild(a)
      setSuccess(true); setFiles([])
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to merge images. Please try again.')
    } finally {
      setMerging(false)
    }
  }
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Merge Images</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Combine multiple images into one — vertically or horizontally</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        ...
        {/* Direction toggle */}
        <div style={{ marginBottom:'24px', background:'#f8fafc', border:'1.5px solid #e2e8f0', borderRadius:'12px', padding:'16px 20px' }}>
          <div style={{ fontSize:'13px', fontWeight:700, color:'#64748b', textTransform:'uppercase' as const, letterSpacing:'0.6px', marginBottom:'12px' }}>Layout Direction</div>
          <div style={{ display:'flex', gap:'10px' }}>
            <button
              onClick={() => setDirection('vertical')}
              style={{
                flex:1, padding:'12px 16px', borderRadius:'10px', border:'2px solid',
                borderColor: direction === 'vertical' ? '#E85D04' : '#e2e8f0',
                background: direction === 'vertical' ? '#FFF7ED' : 'white',
                color: direction === 'vertical' ? '#E85D04' : '#64748b',
                cursor:'pointer', fontFamily:'inherit', fontWeight:700, fontSize:'14px',
                display:'flex', alignItems:'center', justifyContent:'center', gap:'8px',
                transition:'all 0.15s',
              }}>
              <span style={{ fontSize:'18px' }}>↕️</span> Vertical (stacked)
            </button>
            <button
              onClick={() => setDirection('horizontal')}
              style={{
                flex:1, padding:'12px 16px', borderRadius:'10px', border:'2px solid',
                borderColor: direction === 'horizontal' ? '#E85D04' : '#e2e8f0',
                background: direction === 'horizontal' ? '#FFF7ED' : 'white',
                color: direction === 'horizontal' ? '#E85D04' : '#64748b',
                cursor:'pointer', fontFamily:'inherit', fontWeight:700, fontSize:'14px',
                display:'flex', alignItems:'center', justifyContent:'center', gap:'8px',
                transition:'all 0.15s',
              }}>
              <span style={{ fontSize:'18px' }}>↔️</span> Horizontal (side by side)
            </button>
          </div>
        </div>
        ...
        <div
          ...
          <div style={{ fontSize:'12px', color:'#94a3b8', marginTop:'12px' }}>JPG, PNG, WebP, GIF, BMP · Max 20 MB each · Up to 20 images</div>
        ...
        {files.length > 0 && (
          <div style={{ marginTop:'24px' }}>
            <div style={{ fontSize:'14px', color:'#64748b', marginBottom:'12px', fontWeight:600 }}>
              {files.length} image{files.length !== 1 ? 's' : ''} added — use arrows to reorder
            </div>
            {files.map((file, idx) => (
        ...
        {success && (
          <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>
            ✅ Images merged successfully! Your download has started.
          </div>
        )}

        {/* Merge button */}
        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={mergeImages} disabled={files.length < 2 || merging}
            style={{
            ...
            {merging ? '⏳ Merging images…'
              : files.length < 2
                ? `Add ${2 - files.length} more image${files.length === 0 ? 's' : ''} to continue`
                : `🖼️ Merge ${files.length} Images`}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        ...
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Merge Images</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {[
              'Choose a layout direction — Vertical (stacked top to bottom) or Horizontal (side by side).',
              'Click "Select Images" or drag your image files into the upload area.',
              'Add at least 2 images (up to 20 at a time).',
              'Reorder them using the ↑ and ↓ arrows.',
              'Click the orange "Merge Images" button and your merged image downloads automatically.',
            ].map((step, i) => (
        ...
        <section style={{ marginBottom:'40px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))', gap:'14px' }}>
            {[
              { icon:'📸', title:'Before & After', desc:'Place two photos side by side to show a transformation.' },
              { icon:'🗂️', title:'Screenshot Collage', desc:'Stack multiple screenshots into one tall image for sharing.' },
              { icon:'🛍️', title:'Product Comparisons', desc:'Merge product images horizontally for easy comparison.' },
              { icon:'📊', title:'Infographic Assembly', desc:'Combine chart sections into a single shareable image.' },
            ].map(c => (
        ...
        <section>
          ...
          {[
            { q:'What image formats are supported?', a:'JPG, PNG, WebP, GIF, and BMP are all supported. You can mix formats in a single merge.' },
            { q:'What is the difference between vertical and horizontal?', a:'Vertical stacks images top to bottom into a tall image. Horizontal places them side by side into a wide image.' },
            { q:'Is there a file size limit?', a:'Each image can be up to 20 MB. You can merge up to 20 images in one go.' },
            { q:'Are my files kept private?', a:'Yes. Files are sent over HTTPS, processed immediately, and auto-deleted from our servers within 1 hour.' },
            { q:'What format is the output?', a:'The merged image is returned as a PNG to preserve quality across all input formats.' },
          ].map(faq => (
