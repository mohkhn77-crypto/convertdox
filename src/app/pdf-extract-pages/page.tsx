...
export default function PdfExtractPagesPage() {
  ...
  const convert = async () => {
    if (!file) return
    if (!pages.trim()) { setError('Please enter the pages to extract'); return }
    setProcessing(true); setError(''); setSuccess(false)
    try {
      const formData = new FormData()
      formData.append('file', file)
      formData.append('pages', pages.trim())
      const res = await fetch(`${BACKEND_URL}/api/pdf/extract-pages`, { method: 'POST', body: formData })
      if (!res.ok) {
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Extract PDF Pages</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Pull specific pages out of a PDF and save them as a new PDF file</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        ...
        <div style={{ marginTop:'24px', background:'#f8fafc', border:'1.5px solid #e2e8f0', borderRadius:'12px', padding:'20px' }}>
          <label style={{ fontSize:'13px', fontWeight:700, color:'#0F2A4A', display:'block', marginBottom:'6px' }}>Pages to extract</label>
          ...
          <div style={{ fontSize:'12px', color:'#64748b', marginTop:'8px' }}>Pages to extract into new PDF (e.g., 1,3,5-7)</div>
        </div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Done! Your extracted PDF has downloaded.</div>}
        ...
        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={convert} disabled={!file || processing}
            ...
            {processing ? '⏳ Extracting pages…' : '✂️ Extract Pages'}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Extract Pages from a PDF</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload the source PDF.','Enter which pages to extract (e.g., "1,3,5-7").','Click "Extract Pages" — those pages become a new PDF.','Download your extracted PDF.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'What is the difference between Extract and Split?', a:'Split divides a PDF into multiple files. Extract creates one new PDF containing only the pages you specify.' },
            { q:'Can I extract a range of pages?', a:'Yes — use a hyphen for ranges: "5-10" extracts pages 5 through 10 inclusive.' },
            { q:'Does the original PDF stay intact?', a:'Yes — the tool reads your PDF, creates a new one, and leaves the original unchanged.' },
