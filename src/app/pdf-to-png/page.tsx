...
export default function PdfToPngPage() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/pdf/to-png`, { method: 'POST', body: formData })
      if (!res.ok) {
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>PDF to PNG</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Convert each PDF page to a high-quality PNG image</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Converted successfully! Your ZIP of PNG images has downloaded.</div>}
        ...
        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={convert} disabled={!file || processing}
            ...
            {processing ? '⏳ Converting…' : '🖼️ Convert to PNG'}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Convert PDF to PNG</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your PDF by clicking "Select PDF" or dragging it into the area above.','Click "Convert to PNG" — our server renders each page as a high-quality image.','A ZIP file containing one PNG per page downloads automatically.','Open the ZIP to find your images named page-1.png, page-2.png, etc.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'What image quality do I get?', a:'Pages are rendered at 150 DPI by default — crisp enough for most uses. Higher DPI options are planned.' },
            { q:'Does it convert all pages?', a:'Yes — every page in your PDF becomes a separate PNG image, all bundled in a single ZIP download.' },
            { q:'Why choose PNG over JPG?', a:'PNG is a lossless format, making it ideal for PDFs with text, diagrams, or sharp lines. File sizes are larger than JPG but image quality is pixel-perfect.' },
