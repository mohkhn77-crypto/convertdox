...
export default function WebpToJpgPage() {
  ...
  const handleFile = (f: File | null) => {
    if (!f) return
    if (f.type !== 'image/webp' && !f.name.toLowerCase().endsWith('.webp')) {
      setError('Only WebP files are accepted'); return
    }
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/image/webp-to-jpg`, { method: 'POST', body: formData })
      if (!res.ok) {
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        <div
          ...
          <div style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'20px', fontWeight:700, color:'#0F2A4A', marginBottom:'6px' }}>Drop your WebP file here</div>
          <div style={{ fontSize:'14px', color:'#64748b', marginBottom:'18px' }}>or click to browse from your computer</div>
          <button style={{ background:'#E85D04', color:'white', padding:'12px 32px', borderRadius:'10px', border:'none', fontSize:'14px', fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>Select WebP File</button>
          <div style={{ fontSize:'12px', color:'#94a3b8', marginTop:'12px' }}>.webp files only · Max 20 MB</div>
          <input ref={fileInputRef} type="file" accept=".webp,image/webp" style={{ display:'none' }} onChange={e => handleFile(e.target.files?.[0] ?? null)} />
        </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Convert WebP to JPG</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your WebP image file.','Click "Convert WebP to JPG".','Download your converted JPG — works in all apps and platforms.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'Why convert WebP to JPG?', a:'WebP is optimized for the web but many image editors, email clients, and older apps don\'t support it. JPG works everywhere.' },
            { q:'Does WebP to JPG conversion lose quality?', a:'A small amount of quality loss is expected since JPG is lossy, but at high quality settings it is visually indistinguishable.' },
            { q:'Will transparency be preserved?', a:'JPG does not support transparency. Any transparent areas will be filled with white background during conversion.' },
            { q:'Is there a file size limit?', a:'WebP images up to 20 MB are supported.' },
          ].map(faq => (
