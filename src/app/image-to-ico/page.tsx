...
export default function ImageToIcoPage() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/image/image-to-ico`, { method: 'POST', body: formData })
      if (!res.ok) {
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Image to ICO Converter</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Create a .ico favicon file from any image — choose your pixel size</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        <div
          ...
          <div style={{ fontSize:'14px', color:'#64748b', marginBottom:'18px' }}>or click to browse — square images give the best favicon</div>
          ...
          <div style={{ fontSize:'12px', color:'#94a3b8', marginTop:'12px' }}>JPG, PNG, WebP · Max 10 MB</div>
        ...
        <div style={{ marginTop:'24px', background:'#f8fafc', border:'1.5px solid #e2e8f0', borderRadius:'12px', padding:'20px' }}>
          <div style={{ fontSize:'13px', fontWeight:700, color:'#0F2A4A', marginBottom:'12px' }}>ICO Size</div>
          <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' as const }}>
            {sizes.map(s => (
              <button key={s} onClick={() => setSize(s)}
                style={{ padding:'10px 20px', borderRadius:'8px', border:`2px solid ${size === s ? '#E85D04' : '#e2e8f0'}`, background: size === s ? '#FFF7ED' : 'white', color: size === s ? '#E85D04' : '#0F2A4A', fontSize:'14px', fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
                {s}×{s}
              </button>
            ))}
          </div>
          <div style={{ fontSize:'12px', color:'#64748b', marginTop:'8px' }}>32×32 is the standard browser favicon size. 16×16 for very small displays.</div>
        </div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Done! Your favicon.ico ({size}×{size}px) has downloaded.</div>}
        ...
        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={convert} disabled={!file || processing}
            ...
            {processing ? '⏳ Converting…' : '⭐ Create ICO File'}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Create a Favicon ICO</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your logo or icon image (square works best).','Select the ICO size (32×32 for standard favicons).','Click "Create ICO File".','Save the favicon.ico to your website\'s root directory and reference it in your HTML <head>.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'What size should I use?', a:'32×32 is the standard browser tab favicon size. Use 16×16 for very small displays. For all sizes at once, use our PNG to ICO tool.' },
            { q:'What format is best as input?', a:'PNG with transparency gives the best results. Square images (1:1 ratio) produce the sharpest favicon.' },
            { q:'Where do I put the favicon.ico?', a:'Place it in the root of your website (same level as index.html) and add: <link rel="icon" href="/favicon.ico"> to your HTML <head>.' },
