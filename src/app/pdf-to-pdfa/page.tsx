...
export default function PdfToPdfaPage() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/pdf/to-pdfa`, { method: 'POST', body: formData })
      if (!res.ok) {
      ...
      a.href = url; a.download = 'archive.pdf'
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          <div style={{ width:'56px', height:'56px', background:'rgba(100,116,139,0.25)', borderRadius:'14px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'28px' }}>📚</div>
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>PDF to PDF/A</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Convert to PDF/A archival format for long-term storage</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'16px auto 0', padding:'0 24px' }}>
        <div style={{ background:'#EFF6FF', border:'1.5px solid #BFDBFE', borderRadius:'10px', padding:'12px 16px', fontSize:'13px', color:'#1e40af', lineHeight:'1.6' }}>
          📚 <strong>What is PDF/A?</strong> PDF/A is an ISO-standardised archival format designed for long-term preservation. It embeds all fonts, colours, and metadata — ensuring documents render identically for decades. Required by many government, legal, and archival institutions.
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'16px auto 0', padding:'0 24px' }}>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Converted to PDF/A! Your archive.pdf has downloaded.</div>}
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Convert to PDF/A</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your standard PDF.','Click "Convert to PDF/A".','Our server embeds all fonts, colour profiles, and metadata to meet the PDF/A standard.','Your archive-ready PDF downloads automatically.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'Which PDF/A version is used?', a:'We output PDF/A-1b, which is the most widely accepted archival version and required by many institutions.' },
            { q:'Will my document look different?', a:'Visually the document should look identical. Embedded transparency and some multimedia elements may be flattened to meet the standard.' },
            { q:'Who needs PDF/A?', a:'Government agencies, law firms, hospitals, universities, and any organization required to retain documents for years or decades typically require PDF/A.' },
