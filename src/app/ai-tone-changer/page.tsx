...
const TONES = [
  ...
  { value:'formal',       label:'Formal',       desc:'Polished and respectful' },
  { value:'confident',    label:'Confident',    desc:'Assertive and decisive' },
...
export default function ToneChanger() {
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch(`${BACKEND}/api/ai/tone`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text, tone }),
      })
      const data = await res.json() as { result?: string; error?: string }
      if (!res.ok) throw new Error(data.error || 'Failed to change tone')
    ...
    <div style={{ minHeight:'100vh', background:'#f8fafc', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'860px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          <div style={{ width:'56px', height:'56px', background:'rgba(236,72,153,0.2)', borderRadius:'14px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'28px' }}>🎭</div>
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>AI Tone Changer</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Adjust the tone of your text to match any context.</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px 48px' }}>
        <div style={{ background:'white', padding:'32px', borderRadius:'16px', boxShadow:'0 1px 3px rgba(0,0,0,0.05)' }}>
          <label style={{ display:'block', fontSize:'14px', fontWeight:700, color:'#0F2A4A', marginBottom:'8px' }}>Text to rewrite</label>
          <textarea value={text} onChange={e => { setText(e.target.value); setError('') }}
            placeholder="Enter the text you want to change the tone of..."
          ...
          <div style={{ marginTop:'20px' }}>
            <label style={{ display:'block', fontSize:'14px', fontWeight:700, color:'#0F2A4A', marginBottom:'12px' }}>Target tone</label>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:'10px' }}>
              {TONES.map(t => (
                <button key={t.value} onClick={() => setTone(t.value)}
                  style={{ padding:'12px', borderRadius:'10px', border: tone === t.value ? '2px solid #E85D04' : '1.5px solid #e2e8f0', background: tone === t.value ? '#FFF7ED' : 'white', cursor:'pointer', textAlign:'left' as const, fontFamily:'inherit', transition:'all 0.15s' }}>
                  <div style={{ fontSize:'13px', fontWeight:700, color: tone === t.value ? '#E85D04' : '#0F2A4A', marginBottom:'2px' }}>{t.label}</div>
          ...
          <button onClick={handleSubmit} disabled={loading || text.trim().length < 10}
            style={{ marginTop:'24px', background: loading || text.trim().length < 10 ? '#cbd5e1' : '#E85D04', color:'white', border:'none', padding:'14px 32px', borderRadius:'10px', fontSize:'15px', fontWeight:700, cursor: loading || text.trim().length < 10 ? 'not-allowed' : 'pointer', fontFamily:'inherit' }}>
            {loading ? '⏳ Rewriting…' : '🎭 Change Tone'}
          </button>
          ...
          {result && (
            <div style={{ marginTop:'24px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
                <h3 style={{ fontSize:'16px', fontWeight:700, color:'#0F2A4A', margin:0 }}>Rewritten Text</h3>
