...
export default function UnlockPdfPage() {
  ...
  const unlock = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/pdf/unlock`, { method: 'POST', body: formData })
      if (!res.ok) {
      ...
      a.href = url; a.download = 'unlocked.pdf'
      document.body.appendChild(a); a.click()
      URL.revokeObjectURL(url); document.body.removeChild(a)
      setSuccess(true); setFile(null); setPassword('')
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to unlock PDF. Check the password and try again.')
    } finally {
      setProcessing(false)
    }
  }
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Unlock PDF</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Remove password protection from PDF files</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        ...
        <div style={{ marginTop:'20px' }}>
          <label style={{ display:'block', fontSize:'14px', fontWeight:700, color:'#0F2A4A', marginBottom:'8px' }}>Password (if the PDF is password-protected)</label>
          <input
            type="password"
            value={password}
            onChange={e => setPassword(e.target.value)}
            placeholder="Leave blank if the PDF has no password"
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Unlock a PDF</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your password-protected PDF.','Enter the PDF password in the field below (leave blank if removing print/edit restrictions only).','Click "Unlock PDF".','Your unlocked PDF downloads automatically — open it in any viewer without a password.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'Do I need the password to unlock?', a:'Yes, if the PDF requires a password to open. Some PDFs only restrict editing/printing — those can often be unlocked without the password.' },
            { q:'What if the wrong password is entered?', a:'The server will return an error. Double-check the password and try again.' },
            { q:'Is it legal to unlock a PDF?', a:"You should only unlock PDFs you own or have permission to access. Don't use this tool to bypass security on documents you don't have rights to." },
            { q:'Are my files kept private?', a:'Yes. Files and passwords are sent over HTTPS and deleted from our server within 1 hour.' },
          ].map(faq => (
