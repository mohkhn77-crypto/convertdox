...
export default function PdfDeletePagesPage() {
  ...
  const convert = async () => {
    if (!file) return
    if (!pages.trim()) { setError('Please enter the pages to delete'); return }
    setProcessing(true); setError(''); setSuccess(false)
    try {
      const formData = new FormData()
      formData.append('file', file)
      formData.append('pages', pages.trim())
      const res = await fetch(`${BACKEND_URL}/api/pdf/delete-pages`, { method: 'POST', body: formData })
      if (!res.ok) {
      ...
      a.href = url; a.download = 'pages-deleted.pdf'
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Delete PDF Pages</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Remove specific pages from a PDF — supports ranges and comma-separated lists</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        ...
        <div style={{ marginTop:'24px', background:'#f8fafc', border:'1.5px solid #e2e8f0', borderRadius:'12px', padding:'20px' }}>
          <label style={{ fontSize:'13px', fontWeight:700, color:'#0F2A4A', display:'block', marginBottom:'6px' }}>Pages to delete</label>
          ...
          <div style={{ fontSize:'12px', color:'#64748b', marginTop:'8px' }}>Enter pages to delete: comma-separated or ranges (e.g., 1,3,5-7)</div>
        </div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Done! Your PDF with pages removed has downloaded.</div>}
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Delete Pages from a PDF</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload the PDF you want to edit.','Enter the page numbers to delete (e.g., "2,5,8-10").','Click "Delete Pages" — those pages are removed.','Download your trimmed PDF.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'What format do I use to specify pages?', a:'Use commas for individual pages (1,3,5) and hyphens for ranges (5-9). You can combine both: 1,3,5-9,12.' },
            { q:'Can I delete the first or last page?', a:'Yes, any page including the first and last can be removed.' },
            { q:'What happens if I delete all pages?', a:'The server will return an error — a PDF must have at least one page remaining.' },
