...
const SECURITY_TOOLS = [
  {
    ...
    desc: 'Generate cryptographically secure passwords with custom rules.',
    href: '/password-generator',
    features: ['Cryptographically secure', 'Custom length and character sets', 'Include symbols, numbers, mixed case'],
  },
  {
    title: 'Password Strength Checker',
    desc: 'Test how strong your password is with instant feedback.',
    href: '/password-strength',
    features: ['Strength score and rating', 'Entropy calculation', 'Improvement suggestions'],
  },
  {
    title: 'PIN Generator',
    desc: 'Generate secure PINs and numeric codes of any length.',
    href: '/pin-generator',
    features: ['Custom PIN length', 'Avoid repeated digits option', 'Bulk PIN generation'],
  },
  {
    title: 'Username Generator',
    desc: 'Generate creative random username ideas.',
    href: '/username-generator',
    features: ['Adjective + noun combos', 'Add numbers or symbols', 'Multiple style options'],
  },
] as const
    ...
    <div style={{ minHeight: '100vh', background: '#fff', fontFamily: "'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding: '80px 24px 64px', textAlign: 'center' }}>
        <div style={{ maxWidth: '760px', margin: '0 auto' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '999px', padding: '5px 16px', fontSize: '13px', color: 'rgba(255,255,255,0.85)', marginBottom: '28px' }}>
            <span style={{ background: '#E85D04', borderRadius: '999px', padding: '1px 8px', fontWeight: 700, color: 'white', fontSize: '11px' }}>4 TOOLS</span>
            Security Tools
          </div>
          <h1 style={{ fontFamily: "'Space Grotesk',system-ui,sans-serif", fontSize: 'clamp(24px,5vw,52px)', fontWeight: 800, color: 'white', lineHeight: 1.15, letterSpacing: '-0.5px', margin: '0 0 18px' }}>
            Free Online <span style={{ color: '#F48C42' }}>Security Tools</span>
          </h1>
          <p style={{ fontSize: 'clamp(15px,2vw,18px)', color: 'rgba(255,255,255,0.65)', maxWidth: '540px', margin: '0 auto', lineHeight: 1.7 }}>
            Password generators, strength checkers, PIN generators — all browser-side, never sent to a server.
          </p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '56px 24px' }}>
        <div style={{ marginBottom: '40px', textAlign: 'center' }}>
          <div style={{ fontSize: '12px', fontWeight: 700, color: '#E85D04', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '10px' }}>Security Tools</div>
          <h2 style={{ fontFamily: "'Space Grotesk',system-ui,sans-serif", fontSize: 'clamp(24px,3vw,36px)', fontWeight: 800, color: '#0F2A4A', margin: '0 0 12px' }}>All Security Tools</h2>
          <p style={{ fontSize: '15px', color: '#64748b', margin: 0 }}>4 free security tools — all processing happens in your browser, nothing is sent to any server.</p>
        </div>
