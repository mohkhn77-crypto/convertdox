...
export default function SvgToPngPage() {
  ...
  const handleFile = (f: File | null) => {
    ...
    if (f.type !== 'image/svg+xml' && !f.name.toLowerCase().endsWith('.svg')) {
      setError('Only SVG files are accepted'); return
    }
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/image/svg-to-png`, { method: 'POST', body: formData })
      if (!res.ok) {
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>SVG to PNG Converter</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Convert SVG vector graphics to PNG at any resolution — crisp and pixel-perfect</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        <div
          ...
          <div style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'20px', fontWeight:700, color:'#0F2A4A', marginBottom:'6px' }}>Drop your SVG file here</div>
          <div style={{ fontSize:'14px', color:'#64748b', marginBottom:'18px' }}>or click to browse from your computer</div>
          <button style={{ background:'#E85D04', color:'white', padding:'12px 32px', borderRadius:'10px', border:'none', fontSize:'14px', fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>Select SVG File</button>
          <div style={{ fontSize:'12px', color:'#94a3b8', marginTop:'12px' }}>.svg files only · Max 10 MB</div>
          <input ref={fileInputRef} type="file" accept=".svg,image/svg+xml" style={{ display:'none' }} onChange={e => handleFile(e.target.files?.[0] ?? null)} />
        </div>
        ...
        {file && (
          <div style={{ marginTop:'20px', background:'white', border:'1.5px solid #e2e8f0', borderRadius:'10px', padding:'16px', display:'flex', alignItems:'center', gap:'12px' }}>
            <div style={{ width:'40px', height:'40px', background:'#FFFBEB', borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px', flexShrink:0 }}>🎨</div>
        ...
        <div style={{ marginTop:'24px', background:'#f8fafc', border:'1.5px solid #e2e8f0', borderRadius:'12px', padding:'20px' }}>
          <div style={{ fontSize:'13px', fontWeight:700, color:'#0F2A4A', marginBottom:'12px' }}>Output Width (pixels)</div>
          <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' as const }}>
            {widths.map(w => (
              <button key={w} onClick={() => setWidth(w)}
                style={{ padding:'10px 20px', borderRadius:'8px', border:`2px solid ${width === w ? '#E85D04' : '#e2e8f0'}`, background: width === w ? '#FFF7ED' : 'white', color: width === w ? '#E85D04' : '#0F2A4A', fontSize:'14px', fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
                {w}px
              </button>
            ))}
          </div>
          <div style={{ fontSize:'12px', color:'#64748b', marginTop:'8px' }}>Height scales proportionally based on your SVG viewBox.</div>
        </div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Converted! Your converted.png ({width}px wide) has downloaded.</div>}
        ...
        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={convert} disabled={!file || processing}
            ...
            {processing ? '⏳ Converting…' : '🎨 Convert SVG to PNG'}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Convert SVG to PNG</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your SVG file.','Choose your output width (1024px is great for most uses; 2048px for high-DPI/retina).','Click "Convert SVG to PNG".','Download your crisp PNG file.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'Why convert SVG to PNG?', a:'SVG is not supported in many contexts — Word documents, email clients, older image editors, and some social media platforms. PNG works universally.' },
            { q:'Will I lose the vector quality?', a:'PNG is raster — the quality is fixed at the pixel size you choose. Use 2048px for the sharpest result, especially for printing.' },
            { q:'Is transparency preserved?', a:'Yes — SVG transparency is preserved in the output PNG. PNG supports alpha channels.' },
            { q:'Is there a file size limit?', a:'SVG files up to 10 MB are supported.' },
          ].map(faq => (
