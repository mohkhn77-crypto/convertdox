...
export default function ExcelToPdfPage() {
  ...
  const handleFile = (f: File | null) => {
    ...
    if (!ACCEPTED.includes(f.type) && !f.name.match(/\.(xlsx?|xls)$/i)) {
      setError('Only Excel files (.xlsx, .xls) are accepted'); return
    }
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/pdf/excel-to-pdf`, { method: 'POST', body: formData })
      if (!res.ok) {
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Excel to PDF</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Convert Excel spreadsheets to PDF with formatting preserved</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        <div
          ...
          <div style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'20px', fontWeight:700, color:'#0F2A4A', marginBottom:'6px' }}>Drop your Excel file here</div>
          <div style={{ fontSize:'14px', color:'#64748b', marginBottom:'18px' }}>or click to browse from your computer</div>
          <button style={{ background:'#E85D04', color:'white', padding:'12px 32px', borderRadius:'10px', border:'none', fontSize:'14px', fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>Select Excel File</button>
          <div style={{ fontSize:'12px', color:'#94a3b8', marginTop:'12px' }}>.xlsx and .xls files · Max 50 MB</div>
          <input ref={fileInputRef} type="file" accept=".xlsx,.xls,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel" style={{ display:'none' }} onChange={e => handleFile(e.target.files?.[0] ?? null)} />
        </div>
        ...
        {file && (
          <div style={{ marginTop:'20px', background:'white', border:'1.5px solid #e2e8f0', borderRadius:'10px', padding:'16px', display:'flex', alignItems:'center', gap:'12px' }}>
            <div style={{ width:'40px', height:'40px', background:'#DCFCE7', borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center', color:'#217346', fontWeight:800, fontSize:'11px', flexShrink:0 }}>XLS</div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Convert Excel to PDF</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your .xlsx or .xls file.','Click "Convert to PDF".','Your PDF downloads automatically — all sheets rendered with formatting intact.','Open in any PDF viewer.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'Which Excel formats are supported?', a:'.xlsx (Excel 2007+) and .xls (older Excel) are both supported.' },
            { q:'Are multiple sheets converted?', a:'Yes — all sheets in the workbook are included in the output PDF, each on its own page.' },
            { q:'Will charts and images be preserved?', a:'Standard charts and embedded images are preserved. Complex macros or ActiveX controls will not render.' },
            { q:'Is there a file size limit?', a:'Yes, Excel files up to 50 MB are supported.' },
          ].map(faq => (
