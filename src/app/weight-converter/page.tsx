...
const TO_GRAMS: Record<string, number> = {
  ...
  oz: 28.3495, lb: 453.592, stone: 6350.29, 'us-ton': 907185, 'uk-ton': 1016050,
  carat: 0.2, 'troy-oz': 31.1035,
}

const UNITS: { key: string; label: string }[] = [
  { key: 'mg', label: 'Milligrams (mg)' },
  { key: 'g', label: 'Grams (g)' },
  { key: 'kg', label: 'Kilograms (kg)' },
  { key: 'tonne', label: 'Tonnes (t)' },
  { key: 'oz', label: 'Ounces (oz)' },
  { key: 'lb', label: 'Pounds (lb)' },
  { key: 'stone', label: 'Stone' },
  { key: 'us-ton', label: 'US Ton' },
  { key: 'uk-ton', label: 'UK Ton' },
  { key: 'carat', label: 'Carats (ct)' },
  { key: 'troy-oz', label: 'Troy Ounces (tr oz)' },
]
...
export default function WeightConverterPage() {
  ...
  function convert(targetKey: string): string {
    ...
    if (isNaN(num)) return '—'
    const grams = num * TO_GRAMS[unit]
    const result = grams / TO_GRAMS[targetKey]
    ...
    <div style={{ minHeight: '100vh', background: '#fff', fontFamily: "'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding: '48px 24px 40px' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily: "'Space Grotesk',system-ui,sans-serif", fontSize: 'clamp(24px,3vw,36px)', fontWeight: 800, color: 'white', margin: 0 }}>Weight Converter</h1>
            <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: '15px', margin: '6px 0 0' }}>Convert between all weight and mass units</p>
          </div>
        </div>
      </div>
      <TrustStrip />
