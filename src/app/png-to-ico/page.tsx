...
export default function PngToIcoPage() {
  ...
  const handleFile = (f: File | null) => {
    ...
    if (f.type !== 'image/png' && !f.name.toLowerCase().endsWith('.png')) {
      setError('Only PNG files are accepted'); return
    }
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/image/png-to-ico`, { method: 'POST', body: formData })
      if (!res.ok) {
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>PNG to ICO Converter</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Convert PNG to multiple ICO sizes (16, 32, 48, 64) — all in one ZIP</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'20px auto 0', padding:'0 24px' }}>
        <div style={{ background:'#EFF6FF', border:'1.5px solid #BFDBFE', borderRadius:'12px', padding:'14px 18px' }}>
          <div style={{ fontSize:'14px', fontWeight:700, color:'#1E40AF', marginBottom:'8px' }}>📦 What you get in the ZIP</div>
          <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' as const }}>
            {[16,32,48,64].map(s => (
              <span key={s} style={{ background:'#DBEAFE', color:'#1E40AF', fontSize:'12px', fontWeight:600, padding:'3px 8px', borderRadius:'4px' }}>{s}×{s}px ICO</span>
            ))}
          </div>
          <div style={{ fontSize:'13px', color:'#1E3A8A', marginTop:'8px' }}>Four ICO files covering all standard favicon and application icon sizes.</div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'16px auto 0', padding:'0 24px' }}>
        <div
          ...
          <div style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'20px', fontWeight:700, color:'#0F2A4A', marginBottom:'6px' }}>Drop your PNG file here</div>
          <div style={{ fontSize:'14px', color:'#64748b', marginBottom:'18px' }}>or click to browse — square PNG recommended</div>
          <button style={{ background:'#E85D04', color:'white', padding:'12px 32px', borderRadius:'10px', border:'none', fontSize:'14px', fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>Select PNG File</button>
          <div style={{ fontSize:'12px', color:'#94a3b8', marginTop:'12px' }}>.png files only · Max 10 MB</div>
          <input ref={fileInputRef} type="file" accept=".png,image/png" style={{ display:'none' }} onChange={e => handleFile(e.target.files?.[0] ?? null)} />
        </div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Done! Your favicons.zip (4 ICO sizes) has downloaded.</div>}
        ...
        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={convert} disabled={!file || processing}
            ...
            {processing ? '⏳ Generating ICO files…' : '🌟 Convert PNG to ICO'}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Convert PNG to ICO</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your PNG logo or icon (at least 64×64px recommended).','Click "Convert PNG to ICO" — four sizes are generated instantly.','Download the ZIP and extract the ICO files.','Use the appropriate size in your website or application.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'Why do I need multiple ICO sizes?', a:'Different operating systems and contexts use different sizes: 16×16 for browser tabs, 32×32 for taskbar icons, 48×48 for Windows file icons, 64×64 for higher-DPI displays.' },
            { q:'Can I include ICO in a single file?', a:'ICO files can actually contain multiple sizes in one file. Our tool generates separate files for flexibility — use a tool like ImageMagick to merge them if needed.' },
            { q:'Does transparency transfer to ICO?', a:'Yes — ICO supports transparency, and PNG transparency is preserved in the output.' },
            { q:'Is there a file size limit?', a:'PNG files up to 10 MB are supported.' },
          ].map(faq => (
