...
export default function PdfReorderPagesPage() {
  ...
  const convert = async () => {
    if (!file) return
    if (!order.trim()) { setError('Please enter the new page order'); return }
    setProcessing(true); setError(''); setSuccess(false)
    try {
      const formData = new FormData()
      formData.append('file', file)
      formData.append('order', order.trim())
      const res = await fetch(`${BACKEND_URL}/api/pdf/reorder-pages`, { method: 'POST', body: formData })
      if (!res.ok) {
      ...
      a.href = url; a.download = 'reordered.pdf'
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Reorder PDF Pages</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Rearrange the pages of a PDF into any order you choose</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        ...
        <div style={{ marginTop:'24px', background:'#f8fafc', border:'1.5px solid #e2e8f0', borderRadius:'12px', padding:'20px' }}>
          <label style={{ fontSize:'13px', fontWeight:700, color:'#0F2A4A', display:'block', marginBottom:'6px' }}>New page order</label>
          <input type="text" value={order} onChange={e => setOrder(e.target.value)} placeholder="3,1,2,4"
          ...
          <div style={{ fontSize:'12px', color:'#64748b', marginTop:'8px' }}>Enter new page order (e.g., 3,1,2 puts page 3 first). All pages must be listed.</div>
        </div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Done! Your reordered PDF has downloaded.</div>}
        ...
        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={convert} disabled={!file || processing}
            ...
            {processing ? '⏳ Reordering pages…' : '🔀 Reorder Pages'}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Reorder PDF Pages</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload the PDF whose pages you want to rearrange.','Count the total pages and type the new order (e.g., "3,1,2,4").','Click "Reorder Pages" — the tool rearranges them instantly.','Download your reordered PDF.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'Do I need to include every page?', a:'Yes — the order field must include every page number in your PDF, otherwise the server will return an error.' },
            { q:'Can I duplicate pages?', a:'Yes — entering a page number twice (e.g., 1,1,2,3) will duplicate that page in the output.' },
            { q:'What if I enter a page that does not exist?', a:'The server will return an error message. Make sure your page numbers do not exceed the total page count.' },
