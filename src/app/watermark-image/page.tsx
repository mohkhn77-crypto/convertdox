...
export default function WatermarkImagePage() {
  ...
  const watermark = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/image/watermark`, { method: 'POST', body: formData })
      if (!res.ok) {
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Watermark Image</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Add a custom text watermark to protect your images</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        ...
        <div style={{ marginTop:'24px', background:'#f8fafc', border:'1.5px solid #e2e8f0', borderRadius:'12px', padding:'20px' }}>
          <label style={{ fontSize:'13px', fontWeight:700, color:'#0F2A4A', display:'block', marginBottom:'6px' }}>Watermark Text</label>
          <input type="text" value={text} onChange={e => setText(e.target.value)} placeholder="WATERMARK"
          ...
          <div style={{ fontSize:'12px', color:'#64748b', marginTop:'8px' }}>The watermark will be placed diagonally across the center of the image at 50% opacity.</div>
        </div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Done! Your watermarked image has downloaded.</div>}

        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={watermark} disabled={!file || processing}
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Watermark an Image</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your image.','Enter the watermark text (e.g., your name, website, or "© 2026").','Click "Add Watermark" — the text is overlaid diagonally.','Download your watermarked image.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'Where is the watermark placed?', a:'The watermark text is placed diagonally across the center of the image at semi-transparent opacity.' },
            { q:'Can I change the font, size, or color?', a:'Currently the tool uses a standard white semi-transparent font. Custom styling is planned for a future update.' },
            { q:'Will the watermark affect image quality?', a:'The image quality itself is preserved. Only the text overlay is added.' },
