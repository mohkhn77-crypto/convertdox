...
export default function ImageColorPickerPage() {
  ...
  const [result, setResult] = useState<ColorsResult | null>(null)
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/image/colors`, { method: 'POST', body: formData })
      if (!res.ok) {
        const data = await res.json().catch(() => ({ error: 'Unknown error' })) as { error?: string }
        throw new Error(data.error ?? `Server error: ${res.status}`)
      }
      const data = await res.json() as ColorsResult
      setResult(data)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to analyze colors. Please try again.')
    } finally {
      setProcessing(false)
    }
  }
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Image Color Picker</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Extract dominant colors and color palette from any image</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        ...
        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={analyze} disabled={!file || processing}
            ...
            {processing ? '⏳ Analyzing colors…' : '🎨 Extract Colors'}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Extract Colors from an Image</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload any image (photo, logo, screenshot, etc.).','Click "Extract Colors" — our tool analyzes the image pixel data.','View the dominant color and full palette with HEX and RGB values.','Click "Copy" next to any color to copy it to your clipboard.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'How many colors are extracted?', a:'The tool returns the 1 dominant color plus up to 5 palette colors — 6 colors total.' },
            { q:'What is the dominant color?', a:'The dominant color is the most frequently occurring color in the image, determined by pixel sampling.' },
            { q:'Can I use these colors in CSS?', a:'Yes — copy the HEX value and paste it directly into your CSS, or use the RGB value as rgb() in CSS.' },
