...
export default function PdfToTextPage() {
  ...
  const extract = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/pdf/extract-text`, { method: 'POST', body: formData })
      if (!res.ok) {
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>PDF to Text</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Extract all text content from a PDF document</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        <div
          ...
          <div style={{ fontSize:'12px', color:'#94a3b8', marginTop:'12px' }}>PDF files only · Max 50 MB · Text-based PDFs only (not scanned)</div>
        ...
        {file && (
          <div style={{ marginTop:'20px', background:'white', border:'1.5px solid #e2e8f0', borderRadius:'10px', padding:'16px', display:'flex', alignItems:'center', gap:'12px' }}>
            ...
            <button onClick={() => { setFile(null); setText('') }} style={{ background:'#FEE2E2', color:'#DC2626', border:'none', borderRadius:'6px', padding:'4px 10px', cursor:'pointer', fontWeight:700, fontSize:'16px' }}>×</button>
          </div>
        )}
          ...
          <div style={{ marginTop:'28px' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px', flexWrap:'wrap' as const, gap:'8px' }}>
              <div style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'16px', fontWeight:700, color:'#0F2A4A' }}>
                Extracted Text <span style={{ fontSize:'13px', fontWeight:400, color:'#94a3b8' }}>({text.length.toLocaleString()} characters)</span>
              </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section>
          ...
          {[
            { q:'Does this work on scanned PDFs?', a:'No. Scanned PDFs are images — they contain no embedded text. Use our Image to Text (OCR) tool for those.' },
            { q:'Will formatting be preserved?', a:'Plain text extraction loses formatting like bold, tables, and columns. You get the raw text content in reading order.' },
            { q:'Is there a page limit?', a:'No page limit — all pages are extracted. Large PDFs may take a few seconds.' },
