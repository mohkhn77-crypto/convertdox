...
export default function PptxToPdfPage() {
  ...
  const handleFile = (f: File | null) => {
    if (!f) return
    if (!ACCEPTED.includes(f.type) && !f.name.match(/\.(pptx?|ppt)$/i)) {
      setError('Only PowerPoint files (.pptx, .ppt) are accepted'); return
    }
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/pdf/from-pptx`, { method: 'POST', body: formData })
      if (!res.ok) {
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          <div style={{ width:'56px', height:'56px', background:'rgba(198,67,28,0.25)', borderRadius:'14px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'28px' }}>📊</div>
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>PPTX to PDF</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Convert PowerPoint presentations (.pptx, .ppt) to PDF with formatting preserved</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        <div
          ...
          <div style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'20px', fontWeight:700, color:'#0F2A4A', marginBottom:'6px' }}>Drop your PowerPoint file here</div>
          <div style={{ fontSize:'14px', color:'#64748b', marginBottom:'18px' }}>or click to browse from your computer</div>
          <button style={{ background:'#E85D04', color:'white', padding:'12px 32px', borderRadius:'10px', border:'none', fontSize:'14px', fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>Select PowerPoint File</button>
          <div style={{ fontSize:'12px', color:'#94a3b8', marginTop:'12px' }}>.pptx and .ppt files · Max 50 MB</div>
          <input ref={fileInputRef} type="file" accept=".pptx,.ppt,application/vnd.openxmlformats-officedocument.presentationml.presentation,application/vnd.ms-powerpoint" style={{ display:'none' }} onChange={e => handleFile(e.target.files?.[0] ?? null)} />
        </div>
        ...
        {file && (
          <div style={{ marginTop:'20px', background:'white', border:'1.5px solid #e2e8f0', borderRadius:'10px', padding:'16px', display:'flex', alignItems:'center', gap:'12px' }}>
            <div style={{ width:'40px', height:'40px', background:'#FEE2E2', borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center', color:'#C6431C', fontWeight:800, fontSize:'11px', flexShrink:0 }}>PPT</div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Converted successfully! Your PDF has downloaded.</div>}
        ...
        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={convert} disabled={!file || processing}
            ...
            {processing ? '⏳ Converting… (up to 1 minute)' : '📄 Convert to PDF'}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Convert PPTX to PDF</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your PowerPoint presentation (.pptx or .ppt) by clicking "Select PowerPoint File" or dragging it above.','Click the orange "Convert to PDF" button.','Wait while our server renders the presentation — up to 1 minute for large files.','Your PDF downloads automatically with slides, fonts, and images preserved.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'Which PowerPoint formats are supported?', a:'.pptx (PowerPoint 2007 and later) and .ppt (older PowerPoint format) are both supported.' },
            { q:'Will my slide formatting be preserved?', a:'Standard formatting — fonts, slide layouts, images, shapes, and transitions — is preserved as static content. Animations and macros will not render.' },
            { q:'How long does it take?', a:'Most files convert in under 30 seconds. Larger presentations with many images may take up to 1 minute.' },
            { q:'Is there a file size limit?', a:'Yes, PowerPoint files up to 50 MB are supported.' },
            { q:'Are my files kept private?', a:'Yes. Files are sent over HTTPS and auto-deleted from our servers within 1 hour.' },
          ].map(faq => (
