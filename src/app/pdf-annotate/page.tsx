...
export default function PdfAnnotatePage() {
  ...
  const convert = async () => {
    if (!file) return
    if (!annotationText.trim()) { setError('Please enter annotation text'); return }
    setProcessing(true); setError(''); setSuccess(false)
    try {
      const formData = new FormData()
      formData.append('file', file)
      formData.append('annotation', annotationText.trim())
      formData.append('page', pageNum)
      formData.append('x', xPos)
      formData.append('y', yPos)
      const res = await fetch(`${BACKEND_URL}/api/pdf/annotate`, { method: 'POST', body: formData })
      if (!res.ok) {
      ...
      setError(err instanceof Error ? err.message : 'Failed to annotate PDF. Please try again.')
    } finally {
      setProcessing(false)
    }
  }
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Annotate PDF</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Add highlighted text annotations to any page of your PDF</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        ...
        <div style={{ marginTop:'24px', background:'#f8fafc', border:'1.5px solid #e2e8f0', borderRadius:'12px', padding:'20px', display:'flex', flexDirection:'column' as const, gap:'16px' }}>
          <div>
            <label style={{ fontSize:'13px', fontWeight:700, color:'#0F2A4A', display:'block', marginBottom:'6px' }}>Annotation Text</label>
            <input type="text" value={annotationText} onChange={e => setAnnotationText(e.target.value)} placeholder="e.g. Please review this section" style={inputStyle} />
            <div style={{ fontSize:'12px', color:'#64748b', marginTop:'6px' }}>Adds yellow highlighted text at the specified position</div>
          </div>
          ...
          <div style={{ fontSize:'12px', color:'#64748b', background:'#FEFCE8', border:'1px solid #FDE68A', borderRadius:'8px', padding:'10px 14px' }}>
            💡 Position 0,0 = bottom-left of page. Y=700 places the annotation near the top of an A4 page.
          </div>
        </div>
        ...
        {success && <div style={{ marginTop:'16px', background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:'10px', padding:'12px 16px', color:'#166534', fontSize:'14px', fontWeight:600 }}>✅ Annotated! Your PDF has downloaded.</div>}
        ...
        <div style={{ marginTop:'24px', textAlign:'center' as const }}>
          <button onClick={convert} disabled={!file || processing}
            ...
            {processing ? '⏳ Annotating PDF…' : '💬 Annotate PDF'}
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Annotate a PDF</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your PDF.','Type the annotation text you want to add.','Set the page number and X/Y coordinates.','Click "Annotate PDF" and download the result.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'What does the annotation look like?', a:'The text is placed at the specified coordinates with a yellow highlight background — similar to a sticky note or highlighted comment.' },
            { q:'Can I add multiple annotations?', a:'Currently one annotation per run. Process the output again to add more annotations.' },
            { q:'How do I position the annotation accurately?', a:'A4 pages are ~595 × 842 points. Use X=50, Y=700 for the top-left area or X=50, Y=50 for the bottom area.' },
