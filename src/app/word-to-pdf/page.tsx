...
export default function WordToPdfPage() {
  ...
  const ACCEPTED = ['application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/msword']

  const handleFile = (f: File | null) => {
    if (!f) return
    if (!ACCEPTED.includes(f.type) && !f.name.match(/\.(docx?|doc)$/i)) {
      setError('Only Word files (.docx, .doc) are accepted'); return
    }
  ...
  const convert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/pdf/word-to-pdf`, { method: 'POST', body: formData })
      if (!res.ok) {
    ...
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0F2A4A,#1a3a5c)', padding:'48px 24px 40px' }}>
        <div style={{ maxWidth:'1100px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px' }}>
          ...
          <div>
            <h1 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'clamp(24px,3vw,36px)', fontWeight:800, color:'white', margin:0 }}>Word to PDF</h1>
            <p style={{ color:'rgba(255,255,255,0.65)', fontSize:'15px', margin:'6px 0 0' }}>Convert Word documents (.docx, .doc) to PDF with formatting preserved</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:'860px', margin:'32px auto 0', padding:'0 24px' }}>
        <div
          ...
          <div style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'20px', fontWeight:700, color:'#0F2A4A', marginBottom:'6px' }}>Drop your Word file here</div>
          <div style={{ fontSize:'14px', color:'#64748b', marginBottom:'18px' }}>or click to browse from your computer</div>
          <button style={{ background:'#E85D04', color:'white', padding:'12px 32px', borderRadius:'10px', border:'none', fontSize:'14px', fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>Select Word File</button>
          <div style={{ fontSize:'12px', color:'#94a3b8', marginTop:'12px' }}>.docx and .doc files · Max 50 MB</div>
          <input ref={fileInputRef} type="file" accept=".docx,.doc,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/msword" style={{ display:'none' }} onChange={e => handleFile(e.target.files?.[0] ?? null)} />
        </div>
        ...
        {file && (
          <div style={{ marginTop:'20px', background:'white', border:'1.5px solid #e2e8f0', borderRadius:'10px', padding:'16px', display:'flex', alignItems:'center', gap:'12px' }}>
            <div style={{ width:'40px', height:'40px', background:'#DBEAFE', borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center', color:'#2B579A', fontWeight:800, fontSize:'13px', flexShrink:0 }}>W</div>
      ...
      <div style={{ maxWidth:'860px', margin:'48px auto 0', padding:'0 24px 48px' }}>
        <section style={{ marginBottom:'40px' }}>
          <h2 style={{ fontFamily:"'Space Grotesk',system-ui,sans-serif", fontSize:'26px', fontWeight:800, color:'#0F2A4A', marginBottom:'16px' }}>How to Convert Word to PDF</h2>
          <ol style={{ paddingLeft:'24px', fontSize:'15px', color:'#64748b', lineHeight:'1.8' }}>
            {['Upload your Word document (.docx or .doc) by clicking "Select Word File" or dragging it above.','Click the orange "Convert to PDF" button.','Wait while our server renders the document — up to 1 minute for large files.','Your PDF downloads automatically with fonts, layout, and images preserved.'].map((s,i) => (
        ...
        <section>
          ...
          {[
            { q:'Which Word formats are supported?', a:'.docx (Word 2007 and later) and .doc (older Word format) are both supported.' },
            { q:'Will my formatting be preserved?', a:'Standard formatting — fonts, headings, tables, images, and page breaks — is preserved. Complex macros and embedded objects may not render.' },
            { q:'How long does it take?', a:'Most files convert in under 30 seconds. Larger documents with many images may take up to 1 minute.' },
            { q:'Is there a file size limit?', a:'Yes, Word files up to 50 MB are supported.' },
